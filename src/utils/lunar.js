...
function elementSheng(de) { for (const k in SHENG) if (SHENG[k] === de) return k } // 生我
function elementKe(de) { for (const k in KE) if (KE[k] === de) return k }           // 克我
function getElementOfGan(char) {
  const i = GAN.indexOf(char)
  return i >= 0 ? GAN_WUXING[i] : null
}
function isYangGan(char) {
  const i = GAN.indexOf(char)
  return i >= 0 && i % 2 === 0
}
function getTenGod(dayGan, otherGan) {
  if (!otherGan) return ''
  const de = getElementOfGan(dayGan)
  const oe = getElementOfGan(otherGan)
  if (!de || !oe) return ''
  const sameYin = isYangGan(dayGan) === isYangGan(otherGan)
  if (oe === de) return sameYin ? '比肩' : '劫财'
  if (oe === SHENG[de]) return sameYin ? '食神' : '伤官'        // 我生
  if (oe === elementSheng(de)) return sameYin ? '偏印' : '正印' // 生我（同性偏印/异性正印）
  if (oe === KE[de]) return sameYin ? '偏财' : '正财'           // 我克（同性偏财/异性正财）
  if (oe === elementKe(de)) return sameYin ? '七杀' : '正官'    // 克我（同性七杀/异性正官）
  return ''
}
function getTenGods(dayGan, pillars) {
  const labels = ['年', '月', '日', '时']
  return pillars.filter(Boolean).map((p, i) => ({
    label: labels[i],
    gan: p[0],
    god: i === 2 ? '日主' : getTenGod(dayGan, p[0]),
  }))
}
...
function getYongShen(wuxing) {
  ...
  let weak = null, weakN = Infinity, strong = null, strongN = -Infinity
  for (const k of Object.keys(counts)) {
    if (counts[k] < weakN) { weakN = counts[k]; weak = k }
    if (counts[k] > strongN) { strongN = counts[k]; strong = k }
  }
  // 喜用：缺则补缺，俱全则补最弱；忌：过旺者
  const xi = wuxing.missing.length ? wuxing.missing : [weak]
  return {
    xi,
    ji: [strong],
    weak,
    strong,
    naming: xi.map(e => ({ element: e, chars: NAMING_CHARS[e] || [] })),
  }
}
